...
4. AJAX-обработчик `bspb_ep_ajax_create_payment` ([bspb-elementor-payment.php](bspb-elementor-payment.php))
   ...
   редирект.
5. После успешной генерации ссылки `bspb_ep_notify_admin()` шлёт письмо
   администратору (`wp_mail`) на адрес из настроек (`notify_email`, по умолчанию
   `admin_email`; пусто — не слать). Это сигнал о НАЧАЛЕ оплаты, не о завершении —
   вебхука подтверждения платежа в плагине нет.
...
- **Цена только с сервера.** Никогда не доверяй сумме/цене из запроса клиента.
  ...
  (`bspb_ep_store_options()` → `bspb_ep_get_stored_payload()`). Клиент шлёт лишь
  индекс. Это защита от подмены суммы. Значение доп. поля-списка тоже валидируется
  по серверному списку (`select_values` в payload), телефон — свободный ввод
  (санитизация), оба попадают в `description` заказа (в SDK отдельных полей нет).
